...
void loop()
{
  ...
  //Serial.println(((delta4-delta3)-(delta2 - delta)));
  ...
  if (force > 260) {  // if someone is sitting
    someoneissitting();
  } else if (rangeInCentimeters > 20 && rangeInCentimeters < 240) { // anyone within 2m,
    glow3();
...
void ultrasonic() {
  ...
  //    int delta[i];
  //    int uS[i] = sonar.ping();
  //    delta(i) = (uS(i) / US_ROUNDTRIP_CM;
  //  }

  int uS8 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta8 = (uS8 / US_ROUNDTRIP_CM);
  delay(1);

  int uS7 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta7 = (uS7 / US_ROUNDTRIP_CM);
  delay(1);

  int uS6 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta6 = (uS6 / US_ROUNDTRIP_CM);
  delay(1);

  int uS5 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta5 = (uS5 / US_ROUNDTRIP_CM);
  delay(1);

  int uS4 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta4 = (uS4 / US_ROUNDTRIP_CM);
  delay(1);

  int uS3 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta3 = (uS3 / US_ROUNDTRIP_CM);
  delay(1);

  int uS2 = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta2 = (uS2 / US_ROUNDTRIP_CM);
  delay(1);

  int uS = sonar.ping(); // Send ping, get ping time in microseconds. - Code from IDEA9102 Studio (Happyface Arduino)

  delta = (uS / US_ROUNDTRIP_CM);
  delay(1);

  //
  if (((delta8 - delta7) - (delta6 - delta5)) - ((delta4 - delta3) - (delta2 - delta)) > -5 && ((delta8 - delta7) - (delta6 - delta5)) - ((delta4 - delta3) - (delta2 - delta)) < 5) {

    rangeInCentimeters = delta2;
  }
  else if (((delta8 - delta7) - (delta6 - delta5)) - ((delta4 - delta3) - (delta2 - delta)) < -5 || ((delta8 - delta7) - (delta6 - delta5)) - ((delta4 - delta3) - (delta2 - delta)) > 5) {
  ...
  //if ((sonar.ping_cm() - sonar.ping_cm())-(sonar.ping_cm()-sonar.ping_cm()) > 5 || (sonar.ping_cm() - sonar.ping_cm())-(sonar.ping_cm()-sonar.ping_cm()) < -5){
  //  rangeInCentimeters=0;
  //}
